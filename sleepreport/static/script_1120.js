   ...
   document.getElementById('aver_sleep_hours').textContent = average_sleep_hour;
    ...
    switch (emotion_input) {
        ...
        case 'easy':
            ...
            emotionText3.innerText = '이번 주 "바이오피리아(Biophilia) 경험"을 해보세요. 자연 속에서 시간을 보내되, 단순한 산책을 넘어서세요. 환경과 상호 작용하세요 - 나무 껍질을 만지고, 나뭇잎의 질감을 느끼거나 맨발로 풀밭을 걷는 등. 이러한 자연과의 깊은 연결은 치료적일 수 있습니다.';
            break;
        case 'tired':
    ...
    if (activityTitle && activityImg && activityText) {
        ...
        switch (activityInput) {
            case 'phone':
                ...
                activityText3.textContent = '오늘 왼쪽으로 주무세요! 왼쪽으로 자는 것은 소화에 더 좋을 수 있고, 위가 식도보다 낮게 위치하게 되어 위산이 목구멍으로 올라가는 것을 방지해 심장 쓰림 증상을 줄일 수 있습니다.';
                break;
            case 'nicotine':
...
for (let i = 0; i < daysOfWeek.length; i++) {
    ...
    advice.push("주중과 주말 수면 패턴에 차이가 있으시네요!");
    advice.push("주중과 주말 수면 패턴에 차이가 있는 것을 수면의 사회적 제트래그(social jet lag) 현상이라고 합니다. 이 현상은 사람들이 주중과 주말 사이에 서로 다른 수면 스케줄을 가질 때 발생합니다. 예를 들어, 주중에는 일찍 일어나지만 주말에는 늦게까지 잠을 자는 경우, 이로 인해 내부 시계와 외부 환경 사이의 불일치가 발생합니다. 이러한 수면 패턴은 사실상 시간대를 이동하는 것과 유사한 효과를 가져와, 마치 여행으로 인한 제트래그처럼 몸과 마음에 영향을 줄 수 있습니다. 사회적 제트래그는 피로, 집중력 저하, 기분 변화 등을 유발할 수 있으며, 장기적으로는 건강에 부정적인 영향을 미칠 수 있습니다.");
    advice.push("이번 주엔 주중과 주말의 수면 패턴 차이를 줄여보면 어떨까요?");
}
