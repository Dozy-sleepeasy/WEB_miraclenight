   ...
   document.getElementById('aver_sleep_hours').textContent = average_sleep_hour;
    ...
    switch (emotion_input) {
        ...
        case 'sad':
            ...
            emotionText3.innerText = '오늘 "색상 호흡"을 시도해보세요. 진정 효과가 있는 색상(파란색 또는 녹색)을 들이마시고, 스트레스가 되는 색상(빨간색 또는 회색)을 내쉰다고 상상해보세요. 숨을 들이쉴 때마다 몸 전체로 진정 색상이 퍼지고, 숨을 내쉴 때마다 슬픔이 빠져나가는 것을 시각화해보세요.';
            break;
        case 'nervous':
        ...
        case 'cold':
            ...
            emotionText3.innerText = '수분 섭취 유지: 몸이 탈수되지 않도록 충분한 수분을 섭취하는 것이 중요합니다. 물, 허브 차, 국물 등을 마시면 좋습니다. 건강한 식사: 균형 잡힌 식사를 통해 필요한 영양소를 섭취하세요. 신선한 과일과 채소를 포함한 식단은 면역 체계를 강화하는 데 도움이 됩니다. 스팀 흡입: 코가 막힌 경우, 스팀을 흡입하는 것이 호흡을 돕고 코와 기관지를 맑게 하는 데 도움이 될 수 있습니다. 따뜻한 물에 얼굴을 대고 수건으로 덮어 스팀을 흡입하세요.';
            break;
        case 'headache':
            ...
            emotionText3.innerText = '두통을 완화하기 위해서는 수면 환경을 개선하는 것이 중요합니다. 편안한 베개와 매트리스를 사용하고, 방의 조명을 부드럽게 하며, 소음을 최소화하는 것이 좋습니다. 또한, 취침 전 가벼운 스트레칭이나 명상을 통해 몸과 마음을 이완시키는 것도 도움이 됩니다. 수분 섭취를 늘리고, 저녁에 카페인이나 알코올 섭취를 줄이는 것도 두통 완화에 도움이 됩니다. 그리고, 만약 두통이 지속된다면 전문 의료인과 상담하는 것이 중요합니다.';
            break;
        default:
    ...
    if (activityTitle && activityImg && activityText) {
        ...
        switch (activityInput) {
            case 'phone':
                ...
                activityText3.textContent = '4-7-8 호흡법을 시도해 보세요. 호흡을 조절하여 긴장을 풀고 마음을 진정시키는 테크닉입니다. 4초 동안 숨을 들이마시고, 7초 동안 숨을 참은 다음, 8초 동안 천천히 숨을 내쉽니다. 이 과정을 반복하면 긴장이 풀리고 잠이 올 수 있습니다.';
                break;
            case 'nicotine':
...
if (sleepPattern < 70) {
    ...
    advice.push("수면 패턴에 대한 고민이 있으신가요?");
    advice.push("연구에 따르면, 일부 사람들은 유전적으로 '아침형 인간' 또는 '저녁형 인간'으로 기울어져 있습니다. 이는 PER3 유전자의 변이와 관련이 있으며, 이 유전자는 수면-각성 주기에 영향을 미칩니다.");
    advice.push("내가 어떤 유형의 사람인지 확인해보고 나의 수면 패턴에 더 잘 맞는 일상을 만들어 보면 어떨까요?");
}
